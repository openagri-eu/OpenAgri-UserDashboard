...
const SignUpPage = () => {
    ...
    const { fetchData, loading, response, error } = useFetch<{ success: boolean; message: string }>(
        "register/",
        {
            method: "POST",
            body: {
    ...
    useEffect(() => {
        if (response) {
            showSnackbar("success", response.message ?? "Registration successful. Please log in.");
