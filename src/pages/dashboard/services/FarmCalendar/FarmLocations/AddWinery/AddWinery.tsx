...
import { AddWineryProps } from "./AddWinery.types";
...
const AddWinery: React.FC<AddWineryProps> = ({ onAction }) => {
    ...
    const handlePost = () => {
        ...
        // Ensure category is always 'Winery'
    ...
    useEffect(() => {
        if (response) {
            onAction && onAction();
            showSnackbar('success', "Winery added successfully");
        }
    }, [response]);
            ...
            <Box display={'flex'} flexDirection={'column'} gap={2}>
                ...
                <Stack direction={'column'} spacing={2} >
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center">
                        <TextField fullWidth margin="normal" label="Winery identifier" name="identifier" value={formData?.identifier ?? ''} onChange={handleChange} required={isReq('identifier')} error={isReq('identifier') && fieldEmpty('identifier')} />
                        <TextField fullWidth margin="normal" label="Winery type" name="category" value="Winery" disabled />
                    </Stack>
                    <TextField fullWidth margin="normal" multiline rows={3} label="Winery description" name="description" value={formData?.description ?? ''} onChange={handleChange} />
                    <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center">
