...
import { Box, Card, CardContent, Chip, Divider, Skeleton, SvgIcon, Typography } from "@mui/material";
...
const getDetailsColor = (status?: SprayStatus): ChipProps['color'] => {
    switch (status) {
        case 'unsuitable':
            return 'error';
        default:
                                ...
                                <CardContent>
                                    ...
                                    <Box display={'flex'} flexDirection={'column'} gap={2} sx={{ overflowX: 'auto', py: 1 }}>
                                        {Object.entries(dailyData).sort().map(([time, timeData]: [string, TimeData]) => {
                                            const sprayConditions = timeData.spray_conditions ? timeData.spray_conditions?.charAt(0).toUpperCase() + timeData.spray_conditions?.slice(1) : '';
                                            return <Fragment key={`id-time-${date}-${time}`}>
                                                <Box display={'flex'} flex={1} justifyContent={'space-between'} alignItems={'center'} gap={2} sx={{ minWidth: 800 }}>
                                                    ...
                                                    <Box display={'flex'} flex={1} justifyContent={'center'}>
                                                        <Chip label={sprayConditions} color={getStatusColor(timeData.spray_conditions)} />
                                                    </Box>
                                                    <Divider orientation="vertical" flexItem />
                                                    <Box display={'flex'} flex={1} justifyContent={'center'}>
                                                        <Chip label="Humidity" color={getDetailsColor(timeData.detailed_status?.humidity_status)} />
                                                    </Box>
                                                    <Box display={'flex'} flex={1} justifyContent={'center'}>
                                                        <Chip label="Precipitation" color={getDetailsColor(timeData.detailed_status?.precipitation_status)} />
                                                    </Box>
                                                    <Box display={'flex'} flex={1} justifyContent={'center'}>
                                                        <Chip label="Temperature" color={getDetailsColor(timeData.detailed_status?.temperature_status)} />
                                                    </Box>
                                                    <Box display={'flex'} flex={1} justifyContent={'center'}>
                                                        <Chip label="Wind" color={getDetailsColor(timeData.detailed_status?.wind_status)} />
                                                    </Box>
                                                </Box>
                                            </Fragment>
                                        })}
